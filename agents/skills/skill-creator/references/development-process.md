...
```bash
mkdir -p {BASE_PATH}/skills/my-skill/{references,scripts,assets}
touch {BASE_PATH}/skills/my-skill/SKILL.md
```

where `{BASE_PATH}` is wherever skills are stored for current setup
